...
public extension RFKVisuals {
    ...
    struct DominantColor: Identifiable, Hashable, Equatable, Sendable {
        public let id = UUID()
    ...
    static func extractDominantColors(_ k: Int, image platformImage: PlatformImage) async throws -> [DominantColor] {
        ...
        #if os(macOS)
        guard let image = platformImage.cgImage(forProposedRect: nil, context: nil, hints: nil) else {
